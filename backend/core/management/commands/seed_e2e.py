    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            company, _ = Company.objects.get_or_create(
                license_code="E2ETST", defaults={"name": "E2E Testing Corp"}
            )

            user = User.objects.filter(username="e2e_admin").first()
            if not user:
                user = User.objects.create_user(
                    username="e2e_admin", email="admin@e2e.test", password="password"
                )
            ...
            loc_main, _ = Location.objects.get_or_create(
                company=company, name="E2E Main Warehouse", defaults={"type": "WAREHOUSE"}
            )
            loc_secondary, _ = Location.objects.get_or_create(
                company=company, name="E2E Secondary Storage", defaults={"type": "WAREHOUSE"}
            )
            loc_virtual, _ = Location.objects.get_or_create(
                company=company, name="E2E Virtual Trash", defaults={"type": "VIRTUAL"}
            )
            self.stdout.write(
                self.style.SUCCESS(f"Locations seeded: {loc_main}, {loc_secondary}, {loc_virtual}")
            )

            # === 3. Calculators ===
            calc_bucket, _ = CalculatorTemplate.objects.get_or_create(
                company=company,
                name="E2E Bucket Calculator",
                defaults={"engine_type": "bucket", "engine_config": {}},
            )
            calc_tracker, _ = CalculatorTemplate.objects.get_or_create(
                company=company,
                name="E2E Tracker Calculator",
                defaults={"engine_type": "tracker", "engine_config": {}},
            )
            calc_counter, _ = CalculatorTemplate.objects.get_or_create(
                company=company,
                name="E2E Counter Calculator",
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Product Models seeded: {pm_batch}, {pm_serialized}, {pm_standard}"
                )
            )
            ...
            for i in range(1, 4):
                ...
                if not PhysicalProduct.objects.filter(
                    identifier=identifier, product_model=pm_serialized
                ).exists():
            ...
            self.stdout.write(
                self.style.SUCCESS("Physical Products seeded: 3 serialized items via LedgerService")
            )
            ...
            if not ProductBatch.objects.filter(
                batch_identifier="E2E-LOT-001", location=loc_main, product_model=pm_batch
            ).exists():
                from decimal import Decimal
                from inventory.services import LedgerService
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        "Product Batch seeded via LedgerService: E2E-LOT-001 (qty: 100)"
                    )
                )
            else:
