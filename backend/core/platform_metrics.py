...
class PlatformApiUsageView(APIView):
    ...
    def get(self, request):
        ...
        top_consumers = [
            ...
            for row in (
                ApiKey.objects.values(
                    'company_id', 'company__name', 'company__account_type'
                )
        ...
        rate_tier_distribution = {
            tier.value: 0 for tier in ApiKey.RateLimitTier
        }
        for row in (
            ApiKey.objects.values('rate_limit_tier').annotate(n=Count('id'))
        ):
    ...
    def get(self, request):
        ...
        for company in companies:
            ...
            if company.user_count == 0:
                anomalies.append({
                    'kind': 'userless', 'severity': 'warning',
                    'company_id': cid, 'company_name': cname,
            ...
            if company.account_type == manager and company.key_count > 1:
                anomalies.append({
                    'kind': 'multi_key_manager', 'severity': 'warning',
                    'company_id': cid, 'company_name': cname,
        ...
        for key in (
            ApiKey.objects.filter(
                is_active=True, expires_at__gte=now, expires_at__lte=expiring_cutoff,
            ).select_related('company')
        ):
            anomalies.append({
                'kind': 'expiring_key', 'severity': 'warning',
                'company_id': str(key.company_id), 'company_name': key.company.name,
        ...
        for key in (
            ApiKey.objects.filter(is_active=True, expires_at__lt=now)
            .select_related('company')
        ):
            anomalies.append({
                'kind': 'expired_key', 'severity': 'critical',
                'company_id': str(key.company_id), 'company_name': key.company.name,
