...
class RoleEndpointMatrixTests(APITestCase):
    ...
    def _product(self):
        return ProductModel.objects.create(
            company=self.company, sku=f"S-{timezone.now().timestamp()}", name="P",
...
class LicenseExpiryTests(APITestCase):
    def setUp(self):
        ...
        self.product = ProductModel.objects.create(
            company=self.company, sku="EXP-1", name="P", profile="SIMPLE_COUNT",
        )
    ...
    def test_expired_license_blocks_write(self):
        ...
        resp = self.client.post(reverse('productmodel-list'), {
            "name": "New", "sku": "NEW-1", "profile": "SIMPLE_COUNT",
        }, format='json')
        self.assertEqual(resp.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_valid_license_allows_write(self):
        self.client.force_authenticate(user=self.user)
        resp = self.client.post(reverse('productmodel-list'), {
            "name": "New", "sku": "NEW-2", "profile": "SIMPLE_COUNT",
        }, format='json')
    ...
    def test_superuser_bypasses_expiry(self):
        ...
        resp = self.client.post(reverse('productmodel-list'), {
            "name": "New", "sku": "NEW-3", "profile": "SIMPLE_COUNT",
        }, format='json')
        self.assertNotEqual(resp.status_code, status.HTTP_403_FORBIDDEN)
...
class LicenseLimitTests(APITestCase):
    def test_max_users_blocks_sixth_invite(self):
        developer = Company.objects.create(
            name="Dev", account_type=Company.AccountType.DEVELOPER
        )
        child = Company.objects.create(name="Child", parent=developer, max_users=5)
        dev_user = User.objects.create_user(
            username="dev", password="pw", company=developer
        )
        # Seed the child up to its cap (5 users).
        for i in range(5):
            User.objects.create_user(
                username=f"c{i}", password="pw", company=child
            )
        self.client.force_authenticate(user=dev_user)
        resp = self.client.post(reverse('user-invite'), {
            "username": "sixth", "email": "s@x.com", "password": "sup3rsecret",
    ...
    def test_max_products_blocks_create(self):
        ...
        ProductModel.objects.create(
            company=company, sku="C-1", name="P", profile="SIMPLE_COUNT"
        )
        ...
        resp = self.client.post(reverse('productmodel-list'), {
            "name": "Second", "sku": "C-2", "profile": "SIMPLE_COUNT",
        }, format='json')
        self.assertEqual(resp.status_code, status.HTTP_403_FORBIDDEN)
        body = resp.json()
        self.assertEqual(body.get('code'), 'limit_reached')
...
class LicenseRotationTests(APITestCase):
    def test_rotate_invalidates_old_code_at_login(self):
        ...
        User.objects.create_user(
            username="u", password="password123", company=company
        )
        token_url = reverse('token_obtain_pair')

        # Old code authenticates.
        ok = self.client.post(token_url, {
            "username": "u", "password": "password123", "license_code": "OLDCODE",
        }, format='json')
        ...
        bad = self.client.post(token_url, {
            "username": "u", "password": "password123", "license_code": "OLDCODE",
        }, format='json')
        self.assertEqual(bad.status_code, status.HTTP_400_BAD_REQUEST)

        # New code works.
        good = self.client.post(token_url, {
            "username": "u", "password": "password123", "license_code": new_code,
        }, format='json')
