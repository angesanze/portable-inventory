...
def _event(company, *, status_value='OPEN'):
    ...
    product = ProductModel.objects.create(
        company=company, sku=f'EVT-{company.id}-{suffix}', name='Monitored',
    )
    return EventLog.objects.create(
        product=product, message='boom', status=status_value,
    )
...
class TestPlatformApiUsage:
    def test_api_usage_ranks_top_consumers_and_distributes_tiers(self):
        ...
        ApiKey.objects.create(
            company=heavy, key='heavy-1', label='H1',
            usage_count=600, rate_limit_tier=ApiKey.RateLimitTier.PREMIUM,
        )
        ApiKey.objects.create(
            company=heavy, key='heavy-2', label='H2',
            usage_count=400, rate_limit_tier=ApiKey.RateLimitTier.STANDARD,
        )
        ApiKey.objects.create(
            company=light, key='light-1', label='L1',
            usage_count=50, rate_limit_tier=ApiKey.RateLimitTier.STANDARD,
        )
    ...
    def test_api_usage_counts_dormant_keys(self):
        ...
        ApiKey.objects.create(
            company=company, key='stale', label='Stale', last_used_at=old,
        )
        ApiKey.objects.create(
            company=company, key='fresh', label='Fresh',
