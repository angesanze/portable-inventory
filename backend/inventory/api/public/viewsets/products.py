...
from inventory.models import ProductModel, WorkOrder, ProductBatch, PhysicalProduct, Location, Movement
from inventory.services import StockService, LedgerService
from inventory.engines import EngineFactory
from inventory.exceptions import InventoryError, ItemNotFoundError
...
class _ProductListResponseSerializer(serializers.Serializer):
    ...
    poly_products = serializers.ListField(child=serializers.DictField(), help_text="Deprecated, always empty.")


class _TransactionRequestSerializer(serializers.Serializer):
    api_key = serializers.CharField(required=False, help_text="API key (also accepted via X-API-Key header or query param)")
    ...
    direction = serializers.ChoiceField(choices=["Inbound", "Outbound"], required=False, help_text="Stock direction (omit when 'operation' is set)")
    quantity = serializers.DecimalField(max_digits=12, decimal_places=4, required=False, help_text="Quantity to add/subtract (omit when 'operation' is set)")
    ...
    idempotency_key = serializers.CharField(required=False, help_text="Optional key to make a 'fulfill' operation idempotent (safe to retry)")
...
@extend_schema_view(
    ...
    retrieve=extend_schema(
        ...
        parameters=[
            OpenApiParameter(name="api_key", type=str, location="query", description="API key for authentication"),
            OpenApiParameter(name="location_id", type=str, location="query", required=False, description="Filter stock by location"),
        ],
        responses={200: OpenApiResponse(description="Product details with stock info"), 404: OpenApiResponse(description="Product not found")},
    ),
)
class ProductWidgetViewSet(ApiKeyAuthMixin, viewsets.ViewSet):
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter(name="api_key", type=str, location="query", description="API key for authentication"),
            OpenApiParameter(name="code", type=str, location="query", required=True, description="Barcode or QR code string"),
            OpenApiParameter(name="location_id", type=str, location="query", required=False, description="Filter stock by location"),
        ],
        responses={200: OpenApiResponse(description="Product details with stock info"), 404: OpenApiResponse(description="No product matches the code")},
    )
    ...
    def resolve_barcode(self, request):
        ...
        product = (
            ProductModel.objects.filter(company=company, barcode=code)
            .exclude(barcode='')
            .first()
        )
    ...
    @extend_schema(
        ...
        responses={200: _TransactionResponseSerializer, 400: OpenApiResponse(description="Validation error"), 404: OpenApiResponse(description="Product not found"), 409: OpenApiResponse(description="Insufficient stock")},
    )
    ...
    def transaction(self, request, pk=None):
        ...
        if result.get("success") and "product" in result:
             ...
             return Response({
                 "success": True,
                 "new_stock_display": engine.format_stock_display(display_data)
             })
