...
from inventory.models import ProductBatch, PhysicalProduct
...
class InventoryQueryViewSet(ApiKeyAuthMixin, viewsets.ViewSet):
    ...
    @extend_schema(
        ...
        parameters=[
            ...
            OpenApiParameter(
                ...
                location="query",
                required=True,
                description="Product model UUID",
            ),
            OpenApiParameter(
                name="location_id",
                type=str,
        ...
        responses={
            200: _PhysicalItemSerializer(many=True),
            400: OpenApiResponse(description="product_id required"),
        },
    )
    ...
    @extend_schema(
        ...
        responses={
            200: _BatchSerializer(many=True),
            400: OpenApiResponse(description="product_id required"),
        },
    )
    @action(detail=False, methods=["get"])
    def batches(self, request):
        ...
        batches = ProductBatch.objects.filter(
            product_model_id=product_id, product_model__company=api_key.company, quantity__gt=0
