...
class _QRInfoResponseSerializer(serializers.Serializer):
    ...
    target_type = serializers.ChoiceField(
        choices=["PRODUCT", "ITEM", "WORK_ORDER"], allow_null=True
    )
...
class _ConfigureQRRequestSerializer(serializers.Serializer):
    ...
    target_type = serializers.ChoiceField(
        choices=["PRODUCT", "ITEM", "WORK_ORDER"],
        required=False,
        help_text="Target type for Authority Dashboard flow",
    )
    ...
    product_model_id = serializers.UUIDField(
        required=False, help_text="Legacy Widget: product model UUID"
    )
    physical_identifier = serializers.CharField(
        required=False, help_text="Legacy Widget: physical item identifier"
    )
...
class QRCodeWidgetViewSet(ApiKeyAuthMixin, viewsets.ViewSet):
    ...
    @extend_schema(
        ...
        parameters=[
            ...
            OpenApiParameter(
                name="code", type=str, location="query", required=True, description="QR code string"
            ),
        ],
        responses={
            200: _QRInfoResponseSerializer,
        ...
        return Response(
            {
                ...
                "target_id": str(qr.product_model.id)
                if qr.product_model
                else str(qr.physical_product.id)
                if qr.physical_product
                else str(qr.work_order.id)
                if qr.work_order
                else None,
    ...
    @extend_schema(
        ...
        responses={
            200: _QRStatusResponseSerializer,
            404: OpenApiResponse(description="QR code or target not found"),
            409: OpenApiResponse(description="QR code not in VIRGIN state"),
        },
    )
    ...
    def configure_qr(self, request):
        ...
        with transaction.atomic():
            ...
            if target_type == "PRODUCT":
                qr.product_model = get_object_or_404(
                    ProductModel, id=target_id, company=api_key.company
                )
                qr.physical_product = None
                qr.work_order = None
            elif target_type == "ITEM":
                qr.physical_product = get_object_or_404(
                    PhysicalProduct, id=target_id, product_model__company=api_key.company
                )
    ...
    @extend_schema(
        ...
        responses={
            200: _QRStatusResponseSerializer,
            404: OpenApiResponse(description="QR code not found"),
            409: OpenApiResponse(description="QR code not in CONFIGURED state"),
        },
    )
    @action(detail=False, methods=["post"])
    def lock_qr(self, request):
        ...
        api_key = self._validate_api_key(request)
        # Accept both spellings, matching configure_qr — the widget posts `qr_code`.
        code = request.data.get("code") or request.data.get("qr_code")
