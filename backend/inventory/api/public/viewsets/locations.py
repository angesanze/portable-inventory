...
class _CreateLocationRequestSerializer(serializers.Serializer):
    ...
    type = serializers.ChoiceField(
        choices=["WAREHOUSE", "STORE", "TRANSIT"], default="WAREHOUSE", required=False
    )
...
class LocationWidgetViewSet(ApiKeyAuthMixin, viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        return Response([{"id": str(l.id), "name": l.name, "type": l.type} for l in locations])
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(description="Created location with id and name"),
            400: OpenApiResponse(description="Name required"),
        },
    )
    ...
    def create_location(self, request):
        ...
        location = Location.objects.create(company=api_key.company, name=name, type=loc_type)
    ...
    @extend_schema(
        ...
        responses={
            200: _LocationInventoryResponseSerializer,
            400: OpenApiResponse(description="location_id required"),
    ...
    def location_inventory(self, request):
        ...
        return Response({"location": location.name, "contents": contents})
