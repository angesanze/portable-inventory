...
class CountSessionViewSet(CompanyScopedViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if company is None:
            return Response(
                {"detail": "A company context is required."}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def update(self, request, *args, **kwargs):
        ...
        if instance.status == CS_STATUS_APPLIED:
            return Response(
                {"detail": "An applied session is read-only."}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def count(self, request, pk=None):
        ...
        if not isinstance(counts, list) or not counts:
            return Response(
                {"detail": "counts must be a non-empty list."}, status=status.HTTP_400_BAD_REQUEST
            )
        try:
