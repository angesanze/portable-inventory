...
class TransferOrderViewSet(CompanyScopedViewSet):
    ...
    def receive(self, request, pk=None):
        ...
        if not isinstance(receipts, list) or not receipts:
            return Response({"detail": "receipts must be a non-empty list."}, status=status.HTTP_400_BAD_REQUEST)
        try:
    ...
    def shortage(self, request, pk=None):
        ...
        try:
            movements = TransferService.report_shortage(order, line_id, quantity, user=self._user(request))
        except InventoryError as e:
            return Response({"detail": str(e.detail)}, status=e.status_code)
        order.refresh_from_db()
        data = self.get_serializer(order).data
