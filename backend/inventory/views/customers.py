...
from ..models import Customer, Movement
...
class CustomerViewSet(CompanyScopedViewSet):
    """
    ViewSet for Customer (cliente) registry. Company-scoped CRUD.
    """
    ...
    def bulk_delete(self, request):
        """Bulk-delete Customers. Body: {ids, preserve_movements?}."""
        try:
        ...
        with transaction.atomic():
            if preserve_movements:
                preserved = Movement.objects.filter(
                    customer_id__in=scoped_ids,
                ).update(customer=None)
            else:
                Movement.objects.filter(customer_id__in=scoped_ids).delete()
            Customer.objects.filter(id__in=scoped_ids).delete()
