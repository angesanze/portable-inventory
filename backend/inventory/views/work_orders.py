...
class WorkOrderViewSet(CompanyScopedViewSet):
    ...
    def bulk_delete(self, request):
        ...
        with transaction.atomic():
            if cascade:
                batch_ids = list(
                    ProductBatch.objects.filter(work_order_id__in=scoped_ids)
                    .values_list('id', flat=True)
                )
                pp_ids = list(
                    PhysicalProduct.objects.filter(work_order_id__in=scoped_ids)
                    .values_list('id', flat=True)
                )
    ...
    def contents(self, request, pk=None):
        ...
        # 1. Aggregate Batches
        batches = ProductBatch.objects.filter(work_order=work_order, quantity__gt=0).select_related('product_model')
        for b in batches:
        ...
        items = PhysicalProduct.objects.filter(work_order=work_order, status='ACTIVE').select_related('product_model')
        for i in items:
            result.append({
                "type": "SERIAL",
    ...
    def fulfill(self, request, pk=None):
        ...
        work_order = self.get_object()
        try:
            result = WorkOrderFulfillmentService.fulfill(
