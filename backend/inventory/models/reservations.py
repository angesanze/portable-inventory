...
class Reservation(models.Model):
    ...
    product_model = models.ForeignKey(
        ProductModel, on_delete=models.CASCADE, related_name="reservations"
    )
    ...
    location = models.ForeignKey(
        Location, null=True, blank=True, on_delete=models.CASCADE, related_name="reservations"
    )
    batch = models.ForeignKey(
        ProductBatch, null=True, blank=True, on_delete=models.CASCADE, related_name="reservations"
    )
    ...
    reference = models.CharField(
        max_length=255, blank=True, help_text="Free-text purpose (order number, customer, job)."
    )
    expires_at = models.DateTimeField(
        null=True, blank=True, help_text="Auto-release after this moment."
    )

    created_by = models.ForeignKey(
        User, null=True, on_delete=models.SET_NULL, related_name="reservations"
    )
    ...
    def clean(self):
        ...
        if self.physical_product:
            if self.physical_product.product_model.company_id != company_id:
                raise ValidationError(
                    {"physical_product": "physical_product belongs to a different company."}
                )
            if self.quantity != Decimal("1"):
                raise ValidationError(
                    {"quantity": "Serialized reservations are one item at a time."}
                )
        if (
            self.product_model
            and self.product_model.tracking_mode == "INDIVIDUAL"
            and not self.physical_product
        ):
            raise ValidationError(
                {"physical_product": "Serialized products are reserved per physical item."}
            )
