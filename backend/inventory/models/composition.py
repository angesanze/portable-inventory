        ...
        while frontier:
            ...
            if current == self.parent_id:
                raise ValidationError(
                    "Component relationship would create a cycle in the bill of materials."
                )
            if current in visited:
                continue
            visited.add(current)
            frontier.extend(
                ProductComponent.objects.filter(parent_id=current).values_list(
                    "child_id", flat=True
                )
            )
...
class WorkOrder(models.Model):
    ...
    def clean(self):
        ...
        if self.product_model and self.product_model.company_id != self.company_id:
            raise ValidationError(
                {
                    "product_model": "Product model must belong to the same company as the work order."
                }
            )
