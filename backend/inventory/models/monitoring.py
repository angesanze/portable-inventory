...
class MonitoringRule(models.Model):
    ...
    condition_config = models.JSONField(
        default=dict, blank=True, help_text="Config for the rule (e.g. {'threshold': 10})"
    )
...
class EventLog(models.Model):
    ...
    rule = models.ForeignKey(
        MonitoringRule, on_delete=models.SET_NULL, null=True, related_name="events"
    )
    product = models.ForeignKey(ProductModel, on_delete=models.CASCADE, related_name="events")
    batch = models.ForeignKey(
        ProductBatch, on_delete=models.SET_NULL, null=True, blank=True, related_name="events"
    )
