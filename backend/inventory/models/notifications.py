...
def validate_webhook_url(url):
    ...
    if ip is not None and not ip.is_global:
        raise ValidationError(
            {'url': f"Webhook URL must not target a non-public address ({ip})."}
        )
...
class NotificationChannel(models.Model):
    ...
    secret = models.CharField(
        max_length=64, default=generate_webhook_secret, editable=False,
        help_text="HMAC-SHA256 signing key for the X-PI-Signature header.",
    )
    headers = models.JSONField(default=dict, blank=True, help_text="Optional extra HTTP headers (WEBHOOK kind).")
    ...
    def clean(self):
        ...
        if self.kind == 'EMAIL':
            ...
            if not emails:
                raise ValidationError({'recipients': "At least one recipient email address is required."})
            for email in emails:
                try:
