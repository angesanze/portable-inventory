...
class ProductBatch(models.Model):
    ...
    product_model = models.ForeignKey(ProductModel, on_delete=models.CASCADE, related_name='batches')
    location = models.ForeignKey(Location, on_delete=models.CASCADE, related_name='batches')
    batch_identifier = models.CharField(max_length=255, help_text="Unique ID for this batch (e.g. Lot Number)")
    data = models.JSONField(default=dict, blank=True, help_text="Dynamic data based on Strategy Schema")
    quantity = models.DecimalField(max_digits=19, decimal_places=4, default=0)
    work_order = models.ForeignKey('inventory.WorkOrder', on_delete=models.SET_NULL, null=True, blank=True, related_name='batches')
    ...
    def clean(self):
        ...
        if self.location and self.location.company_id != company_id:
            raise ValidationError({'location': "Location belongs to a different company than the product."})
        if self.work_order and self.work_order.company_id != company_id:
            raise ValidationError({'work_order': "Work order belongs to a different company than the product."})
...
class PhysicalProduct(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    product_model = models.ForeignKey(ProductModel, on_delete=models.CASCADE, related_name='physical_products')
    ...
    location = models.ForeignKey(Location, on_delete=models.SET_NULL, null=True, blank=True, related_name='physical_products')
    work_order = models.ForeignKey('inventory.WorkOrder', on_delete=models.SET_NULL, null=True, blank=True, related_name='physical_products')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
