...
class Movement(models.Model):
    ...
    product_model = models.ForeignKey(ProductModel, null=True, blank=True, on_delete=models.SET_NULL, related_name='movements')
    physical_product = models.ForeignKey(PhysicalProduct, null=True, blank=True, on_delete=models.PROTECT, related_name='movements')
    batch = models.ForeignKey(ProductBatch, null=True, blank=True, on_delete=models.PROTECT, related_name='movements')
    work_order = models.ForeignKey(WorkOrder, null=True, blank=True, on_delete=models.SET_NULL, related_name='movements')

    from_location = models.ForeignKey(Location, on_delete=models.PROTECT, related_name='outgoing_movements')
    to_location = models.ForeignKey(Location, on_delete=models.PROTECT, related_name='incoming_movements')
    ...
    supplier = models.ForeignKey(Supplier, null=True, blank=True, on_delete=models.PROTECT, related_name='movements')
    ...
    customer = models.ForeignKey(
        'inventory.Customer', null=True, blank=True,
    ...
    purchase_order_line = models.ForeignKey(
        'inventory.PurchaseOrderLine', null=True, blank=True,
    ...
    performed_by = models.ForeignKey(User, null=True, on_delete=models.SET_NULL, related_name='movements')
    occurred_at = models.DateTimeField(default=timezone.now)
    reason = models.CharField(max_length=255, blank=True)
    
    # Idempotency
    idempotency_key = models.UUIDField(null=True, blank=True, unique=True, help_text="Unique key to prevent duplicate movements.")
    ...
    def clean(self):
        ...
        if self.quantity == 0 and self.from_location_id != self.to_location_id:
            raise ValidationError({'quantity': "Quantity must be positive. Swap 'from' and 'to' for reverse movements."})
    ...
    def _validate_company_consistency(self):
        ...
        if self.from_location and self.from_location.company_id != company.pk:
            raise ValidationError({'from_location': "from_location belongs to a different company."})
        ...
        if self.purchase_order_line and self.purchase_order_line.purchase_order.company_id != company.pk:
            raise ValidationError({'purchase_order_line': "purchase_order_line belongs to a different company."})
