...
class DynamicQRCode(models.Model):
    ...
    api_key = models.ForeignKey('core.ApiKey', on_delete=models.SET_NULL, null=True, blank=True, related_name='qr_codes', help_text="API Key used for widget access")
    
    product_model = models.ForeignKey(ProductModel, null=True, blank=True, on_delete=models.SET_NULL, related_name='qr_codes')
    physical_product = models.ForeignKey(PhysicalProduct, null=True, blank=True, on_delete=models.SET_NULL, related_name='qr_codes')
    batch = models.ForeignKey('inventory.ProductBatch', null=True, blank=True, on_delete=models.SET_NULL, related_name='qr_codes', help_text="Specific batch context")
    work_order = models.ForeignKey('inventory.WorkOrder', null=True, blank=True, on_delete=models.SET_NULL, related_name='qr_codes', help_text="Specific Work Order / Kit context")
    location = models.ForeignKey(Location, null=True, blank=True, on_delete=models.SET_NULL, related_name='qr_codes', help_text="Location context for this QR")
    custom_url = models.URLField(null=True, blank=True, help_text="External URL (overrides product links)")
    ...
    def is_configured(self):
        return bool(self.product_model or self.batch or self.work_order or self.physical_product or self.custom_url)
