...
class ReservationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Reservation
        fields = [
            'id', 'quantity', 'status', 'reference', 'expires_at',
            'created_at', 'updated_at',
            'product_id', 'location_id', 'batch_id', 'physical_product_id',
            'product_sku', 'product_name', 'location_name',
            'batch_identifier', 'physical_identifier',
        ]
    ...
    def validate(self, attrs):
        ...
        if batch_id:
            try:
                attrs['_batch'] = ProductBatch.objects.get(id=batch_id, product_model__company=company)
        ...
        if pp_id:
            try:
                attrs['_physical_product'] = PhysicalProduct.objects.get(id=pp_id, product_model__company=company)
