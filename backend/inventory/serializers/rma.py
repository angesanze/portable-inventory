...
class ReturnOrderSerializer(CompanyScopedOrderSerializerMixin, serializers.ModelSerializer):
    ...
    sales_order_number = serializers.CharField(
        source="sales_order.number", read_only=True, default=None
    )
    ...
    def validate(self, attrs):
        ...
        if purchase_order_id:
            try:
                attrs["purchase_order"] = PurchaseOrder.objects.get(
                    id=purchase_order_id, company=company
                )
        ...
        if not self.instance:
            if kind == constants.RMA_KIND_CUSTOMER_RETURN and "customer" not in attrs:
                raise serializers.ValidationError(
                    {"customer_id": "A customer return requires a customer."}
                )
            if kind == constants.RMA_KIND_SUPPLIER_RETURN and "supplier" not in attrs:
                raise serializers.ValidationError(
                    {"supplier_id": "A supplier return requires a supplier."}
                )
