...
def env(db):
    ...
    warehouse = Location.objects.create(company=company, name="Main Warehouse", type="WAREHOUSE")
    Location.objects.create(company=company, name="External", type="VIRTUAL")
...
def _create_product(env, payload):
    resp = env["app_client"].post("/api/v1/product-models/", payload, format="json")
    assert resp.status_code == 201, resp.content
    return resp.data["id"]


def _widget_list(env):
    resp = env["widget_client"].get("/api/v1/widget/", {"api_key": env["api_key"].key})
...
def _find_in_list(products, sku):
    ...
    assert matches, (
        f"sku {sku!r} not found in widget list (skus={[p.get('sku') for p in products]})"
    )
    return matches[0]
...
def test_perishable_lifecycle(env):
    ...
    expiries = [b.get("expiry_date") for b in detail["stock_value"]]
