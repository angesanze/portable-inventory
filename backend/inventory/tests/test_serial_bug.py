...
class TestSerialRemoval(TestCase):
    def test_remove_serial_from_work_order(self):
        company = Company.objects.create(name="SerialCo1", license_code=uuid.uuid4().hex[:6].upper())
        ...
        self.assertIsNone(serial1.work_order_id, f"Serial work_order_id is {serial1.work_order_id}, should be None")
    ...
    def test_add_bulk_to_work_order(self):
        company = Company.objects.create(name="SerialCo2", license_code=uuid.uuid4().hex[:6].upper())
        model = ProductModel.objects.create(
            company=company,
