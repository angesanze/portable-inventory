...
class ThresholdBucketTests(TestCase):
    ...
    def test_status_low_for_reorder_critical_out(self):
        for sku, qty, expected_bucket in [('S-R', 5, 'REORDER'), ('S-C', 1, 'CRITICAL'), ('S-O', 0, 'OUT')]:
            prod = self._make_product(
                sku,
                reorder_threshold=Decimal('10'),
                critical_threshold=Decimal('2'),
    ...
    def test_status_ok_for_healthy_and_overstock(self):
        ...
        self.assertEqual(
            ProductModelSerializer(healthy).data['stock_status']['status'], 'OK',
        )

        over = self._make_product(
            'OK-O', reorder_threshold=Decimal('10'), max_threshold=Decimal('100'),
        )
        _seed_stock(over, self.warehouse, 150)
        self.assertEqual(
            ProductModelSerializer(over).data['stock_status']['status'], 'OK',
        )
