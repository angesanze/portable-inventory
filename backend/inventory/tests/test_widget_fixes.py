...
def test_transaction_serialized_logic(company_setup, mock_auth):
    ...
    product = ProductModel.objects.create(
        company=company_setup, name="Serial Prod", sku=f"SER-{uuid.uuid4()}", profile="SERIALIZED"
    )
    pp = PhysicalProduct.objects.create(
        product_model=product, identifier=f"SN-{uuid.uuid4()}", location=wh, status="ACTIVE"
    )

    view = ProductWidgetViewSet()
    view.request = MagicMock()
    view.request.data = {"operation": "subtract", "quantity": 1, "physical_product_id": str(pp.id)}
...
def test_transaction_serialized_fifo_fallback(company_setup, mock_auth):
    ...
    pp1 = PhysicalProduct.objects.create(
        product_model=product, identifier=f"SN-OLD-{uuid.uuid4()}", location=wh, status="ACTIVE"
    )
    ...
    view.request.data = {"operation": "subtract", "quantity": 1, "location_id": str(wh.id)}
...
def test_list_location_filtering(company_setup, mock_auth):
    ...
    product = ProductModel.objects.create(
        company=company_setup, name="Prod A", sku=f"SKU-A-{uuid.uuid4()}"
    )
...
@pytest.mark.django_db
def test_list_includes_zero_stock_products_so_they_can_be_loaded(company_setup, mock_auth):
    """Zero-stock products MUST appear in the widget list even with a location
    filter: the widget does check-IN, so a brand-new (zero) product has to be
    selectable to receive its first stock (previously it was hidden, making
    initial loading impossible)."""
    loc = Location.objects.create(name="Loc Zero", type="WAREHOUSE", company=company_setup)
    product = ProductModel.objects.create(
        company=company_setup, name="Prod Zero", sku=f"SKU-Z-{uuid.uuid4()}"
    )
    ...
    with patch.object(ProductWidgetViewSet, "_validate_api_key", return_value=mock_auth):
        with patch("inventory.services.widget_product.StockService") as mock_stock_service:
            ...
            entry = next(p for p in response.data["products"] if p["id"] == str(product.id))
            assert entry["quantity"] == Decimal("0")
