...
class MovementViewSetCreateTest(TestCase):
    ...
    def test_external_vendor_auto_created(self):
        ...
        self.assertFalse(Location.objects.filter(company=company2, name="External Vendor").exists())
        ...
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED)
        self.assertTrue(Location.objects.filter(company=company2, name="External Vendor").exists())
