...
from inventory.models import ProductModel, Location, PhysicalProduct, Reservation
...
def env(db):
    ...
    product = ProductModel.objects.create(
        company=company, sku="RES-BULK", name="Bulk Product",
...
def test_reserve_within_available(env):
    res = ReservationService.reserve(env["product"], 6, env["user"], location=env["warehouse"], reference="SO-1")
...
def test_outbound_respects_reservation(env):
    ...
    with pytest.raises(InsufficientStockError):
        LedgerService.transfer_stock(env["product"], env["warehouse"], env["external"], Decimal("7"), env["user"], "Out")
    LedgerService.transfer_stock(env["product"], env["warehouse"], env["external"], Decimal("4"), env["user"], "Out")
...
def test_fulfilling_transfer_consumes_reservation(env):
    ...
    LedgerService.transfer_stock(
        env["product"], env["warehouse"], env["external"], Decimal("6"), env["user"],
        "Ship SO-1", reservation=res,
    )
    res.refresh_from_db()
...
def test_expired_reservation_ignored(env):
    ReservationService.reserve(
        env["product"], 6, env["user"], location=env["warehouse"],
...
def test_serialized_reserved_item_blocked(env):
    product = ProductModel.objects.create(
        company=env["company"], sku="RES-SER", name="Serialized",
        profile="SERIALIZED",
    )
    pp = PhysicalProduct.objects.create(
        product_model=product, identifier="SN-1", location=env["warehouse"], status="ACTIVE",
    )
    res = ReservationService.reserve(product, 1, env["user"], location=env["warehouse"], physical_product=pp)

    with pytest.raises(InventoryError):
        LedgerService.transfer_stock(
            product, env["warehouse"], env["external"], Decimal("1"), env["user"], "Out",
            physical_product=pp,
        )
    # Fulfilling transfer passes and consumes
    LedgerService.transfer_stock(
        product, env["warehouse"], env["external"], Decimal("1"), env["user"], "Ship",
        physical_product=pp, reservation=res,
    )
    res.refresh_from_db()
    assert res.status == 'CONSUMED'
...
def test_double_reserve_same_item_fails(env):
    product = ProductModel.objects.create(
        company=env["company"], sku="RES-SER2", name="Serialized2", profile="SERIALIZED",
    )
    pp = PhysicalProduct.objects.create(
        product_model=product, identifier="SN-2", location=env["warehouse"], status="ACTIVE",
    )
    ReservationService.reserve(product, 1, env["user"], location=env["warehouse"], physical_product=pp)
    with pytest.raises(InventoryError):
        ReservationService.reserve(product, 1, env["user"], location=env["warehouse"], physical_product=pp)
...
def test_api_crud_and_release(env):
    ...
    resp = client.post('/api/v1/reservations/', {
...
def test_api_cross_company_isolation(env):
    ...
    resp = client.post('/api/v1/reservations/', {
        "product_id": str(env["product"].id), "quantity": "1",
    }, format="json")
