...
def test_retrieve_batches_filtered_by_location(company_setup):
    ...
    product = ProductModel.objects.create(
        company=company, name="Batch Prod", sku=f"BP-{uuid.uuid4()}",
    ...
    ProductBatch.objects.create(
        product_model=product, location=loc_A, quantity=10, 
        batch_identifier="B-IN-A", data={}
    )
    # Batch 2 in B
    ProductBatch.objects.create(
        product_model=product, location=loc_B, quantity=20, 
        batch_identifier="B-IN-B", data={}
    )
