...
def test_data(db):
    ...
    product = ProductModel.objects.create(
        company=company, sku="COFFEE-001", name="Premium Arabica", attributes={"roast": "medium"}
    )

    # Initial Stock
    LedgerService.transfer_stock(
        product, supplier, warehouse, Decimal("100"), user, "Initial Purchase"
    )
...
def test_idempotency(test_data):
    ...
    first = LedgerService.transfer_stock(
        product, warehouse, store, Decimal("10"), user, "Run 1", idempotency_key=key
    )
    ...
    second = LedgerService.transfer_stock(
        product, warehouse, store, Decimal("10"), user, "Run 2", idempotency_key=key
    )
