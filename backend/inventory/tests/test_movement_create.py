...
from inventory.models import (
    Location, Movement, PhysicalProduct, ProductModel,
)
...
class MovementCrossTenantTest(TestCase):
    ...
    def setUp(self):
        ...
        self.product_a = ProductModel.objects.create(
            company=self.company_a, sku="CT-001", name="Product A",
        )
        self.wh_a = Location.objects.create(
            company=self.company_a, name="Warehouse", type="WAREHOUSE",
        )
        self.store_a = Location.objects.create(
            company=self.company_a, name="Store", type="STORE",
        )
        # Location belonging to company B
        self.wh_b = Location.objects.create(
            company=self.company_b, name="Warehouse", type="WAREHOUSE",
        )
    ...
    def test_from_location_other_company_rejected_by_name(self):
        ...
        Location.objects.create(
            company=self.company_b, name="WH-B-Only", type="WAREHOUSE",
        )
    ...
    def test_product_from_other_company_rejected(self):
        ...
        ProductModel.objects.create(
            company=self.company_b, sku="B-ONLY-SKU", name="B Product",
        )
        resp = self.client.post(self.url, {
            "sku": "B-ONLY-SKU",
            "from": "Warehouse",
...
class MovementInsufficientStockTest(TestCase):
    ...
    def setUp(self):
        ...
        self.product = ProductModel.objects.create(
            company=self.company, sku="IS-001", name="Stock Product",
        )
...
class MovementPhysicalProductTest(TestCase):
    ...
    def setUp(self):
        ...
        self.product = ProductModel.objects.create(
            company=self.company, sku="PP-001", name="Serialized Product",
            profile="SERIALIZED",
        )
        self.warehouse = Location.objects.create(
            company=self.company, name="Warehouse", type="WAREHOUSE",
    ...
    def test_physical_product_other_company_rejected(self):
        ...
        product_b = ProductModel.objects.create(
            company=company_b, sku="PP-B01", name="B Product",
            profile="SERIALIZED",
        )
        pp_b = PhysicalProduct.objects.create(
            product_model=product_b, identifier="SN-B-001",
            location=None, status="ACTIVE",
        )
        resp = self.client.post(self.url, {
            "sku": "PP-001",
...
class MovementIdempotencyKeyTest(TestCase):
    ...
    def setUp(self):
        ...
        self.product = ProductModel.objects.create(
            company=self.company, sku="IK-001", name="Idem Product",
        )
        self.vendor, _ = Location.objects.get_or_create(
            company=self.company, name="External Vendor",
            defaults={"type": "VIRTUAL"},
    ...
    def test_null_idempotency_keys_allowed(self):
        ...
        self.assertEqual(
            Movement.objects.filter(
                product_model=self.product, idempotency_key__isnull=True,
            ).count(),
            3,
        )
