...
from inventory.models import ProductModel, Location, Movement, ProductBatch, WorkOrder
...
class WidgetApiTest(APITestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="widget_admin", password="password", company=self.company
        )

        # Setup API Key
        self.api_key = ApiKey.objects.create(
            company=self.company, label="Public Widget Key", key="test-key-123"
        )

        # Setup Products
        self.product1 = ProductModel.objects.create(
            company=self.company, sku="SKU-001", name="Widget A", attributes={"color": "red"}
        )
        self.product2 = ProductModel.objects.create(
            company=self.company, sku="SKU-002", name="Widget B"
        )
        ...
        Movement.objects.create(
            ...
            from_location=Location.objects.create(
    ...
    def test_cross_company_isolation(self):
        ...
        other_product = ProductModel.objects.create(
            company=other_company, sku="OTHER-1", name="Other Widget"
        )
    ...
    def test_list_locations(self):
        ...
        url = reverse(
            "widget-locations"
        )  # Assuming registered as 'widget' -> leads to 'widget-locations' for @action
    ...
    def test_move_validation(self):
        ...
        payload = {"product_id": str(self.product1.id), "location_id": str(self.warehouse.id)}
        response = self.client.post(f"{url}?api_key={self.api_key.key}", payload, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
