...
class SupplierCrudTest(TestCase):
    ...
    def test_create_and_list_supplier(self):
        ...
        names = [s["name"] for s in (listing.data["results"] if isinstance(listing.data, dict) else listing.data)]
...
class ReceiptWithSupplierTest(TestCase):
    def setUp(self):
        ...
        self.product = ProductModel.objects.create(
            company=self.company, sku="RWS-001", name="Bulk Product",
        )
        ...
        self.vendor, _ = Location.objects.get_or_create(
            company=self.company, name="External Vendor", defaults={"type": "VIRTUAL"},
        )
    ...
    def test_supplier_name_exposed_in_list(self):
        self.client.post(self.url, {
            "sku": "RWS-001", "from": "External Vendor", "to": "Warehouse",
            "qty": "5", "supplier_id": str(self.supplier.id),
        })
        listing = self.client.get(self.url)
    ...
    def test_supplier_from_other_company_rejected(self):
        ...
        resp = self.client.post(self.url, {
            "sku": "RWS-001", "from": "External Vendor", "to": "Warehouse",
            "qty": "1", "supplier_id": str(other.id),
        })
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_receipt_without_supplier_still_works(self):
        resp = self.client.post(self.url, {
            "sku": "RWS-001", "from": "External Vendor", "to": "Warehouse", "qty": "3",
        })
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED, resp.data)
...
class ProductOnboardingSupplierTest(TestCase):
    ...
    def test_initial_balance_with_supplier(self):
        resp = self.client.post(self.url, {
            "sku": "POS-001", "name": "Bulk", "profile": "SIMPLE_COUNT",
            "initial_balance": "20", "initial_location_id": str(self.warehouse.id),
    ...
    def test_initial_balance_without_supplier_defaults_external_vendor(self):
        resp = self.client.post(self.url, {
            "sku": "POS-002", "name": "Bulk2", "profile": "SIMPLE_COUNT",
            "initial_balance": "5", "initial_location_id": str(self.warehouse.id),
        }, format="json")
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED, resp.data)
    ...
    def test_initial_balance_not_booked_against_adjustment(self):
        ...
        resp = self.client.post(self.url, {
            "sku": "POS-003", "name": "Bulk3", "profile": "SIMPLE_COUNT",
            "initial_balance": "7", "initial_location_id": str(self.warehouse.id),
        }, format="json")
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED, resp.data)
