...
def setup_ledger_data(db):
    ...
    prod = ProductModel.objects.create(company=company, sku="WIDGET-Err", name="Error Widget", profile="BATCH_TRACKED")
    ...
    loss, _ = Location.objects.get_or_create(company=company, name="Loss", defaults={"type": "LOSS"})
    
    return {"company": company, "product": prod, "location": loc, "supplier": supplier, "loss": loss}
...
def test_consume_insufficient_stock_in_batch(setup_ledger_data):
    ...
    batch = ProductBatch.objects.create(product_model=prod, location=loc, batch_identifier="LOW-STOCK", quantity=5)
