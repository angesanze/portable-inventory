...
class WorkOrderContentsTest(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="woadmin", password="password", company=self.company
        )
        ...
        self.tracker_product = ProductModel.objects.create(
            company=self.company, sku="TRACKER-1", name="Tracker Product", profile="SERIALIZED"
        )
        self.location = Location.objects.create(
            company=self.company, name="Warehouse A", type="WAREHOUSE"
        )
    ...
    def test_empty_work_order(self):
        empty_wo = WorkOrder.objects.create(
            company=self.company, product_model=self.tracker_product, name="Empty Kit"
        )
