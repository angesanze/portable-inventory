...
from inventory.models import Customer, Location, Movement
...
def test_create_and_list(env, api):
    resp = api.post('/api/v1/customers/', {
        "name": "Beta Retail", "email": "ops@beta.test", "phone": "+39 02 1234",
        "address": "Via Roma 1",
    }, format='json')
...
def test_valid_vat_accepted_invalid_rejected(env, api):
    ...
    resp = api.post('/api/v1/customers/', {"name": "Valid VAT", "vat_number": "07643520567"}, format='json')
    assert resp.status_code == 201, resp.content
    resp = api.post('/api/v1/customers/', {"name": "Bad VAT", "vat_number": "12345678901"}, format='json')
...
def test_bulk_delete_preserves_movements(env, api):
    ...
    m = LedgerService.transfer_stock(
        product, warehouse, external, Decimal("1"), env["user"], "Ship", customer=customer,
    )
    assert m.customer == customer

    resp = api.post('/api/v1/customers/bulk-delete/', {
        "ids": [str(customer.id)], "preserve_movements": True,
    }, format='json')
