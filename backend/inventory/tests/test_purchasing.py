...
from inventory.models import (
    Location, Movement, PhysicalProduct, ProductBatch, ProductComponent,
    ProductModel, PurchaseOrder, PurchaseOrderLine, Supplier,
)
from inventory.services import PurchasingService, StockService
from inventory.exceptions import InventoryError
from .helpers import (
    make_assembled_product, make_batch_product, make_serialized_product,
    make_simple_product,
)
...
def make_po(env, lines=None, product=None, qty=Decimal("100"), unit_cost=Decimal("2.50")):
    ...
    for product_model, quantity, cost in lines:
        PurchaseOrderLine.objects.create(
            purchase_order=po, product_model=product_model,
            quantity_ordered=quantity, unit_cost=cost,
        )
    return po
...
def test_numbers_are_sequential_per_company(env):
    ...
    other_po = PurchaseOrder.objects.create(
        company=other_company, supplier=other_supplier,
...
def test_full_cycle_partial_then_complete(env):
    ...
    moves1 = PurchasingService.receive(
        po, [{"line_id": str(line.id), "quantity": "60"}], env["warehouse"], env["user"],
    )
    ...
    moves2 = PurchasingService.receive(
        po, [{"line_id": str(line.id), "quantity": "40"}], env["warehouse"], env["user"],
    )
    po.refresh_from_db(); line.refresh_from_db()
...
def test_over_receipt_blocked(env):
    ...
    with pytest.raises(InventoryError, match="exceeds the remaining"):
        PurchasingService.receive(
            po, [{"line_id": str(line.id), "quantity": "11"}], env["warehouse"], env["user"],
        )
...
def test_repeated_receipts_never_exceed_ordered(env):
    ...
    for _ in range(2):
        PurchasingService.receive(
            po, [{"line_id": str(line.id), "quantity": "5"}], env["warehouse"], env["user"],
        )
    line.refresh_from_db()
    ...
    with pytest.raises(InventoryError):
        PurchasingService.receive(
            po, [{"line_id": str(line.id), "quantity": "1"}], env["warehouse"], env["user"],
        )
    line.refresh_from_db()
    assert line.quantity_received == Decimal("10")
...
def test_confirm_requires_lines(env):
    po = PurchaseOrder.objects.create(
        company=env["company"], supplier=env["supplier"],
        number=PurchasingService.next_number(env["company"]),
    )
...
def test_batch_receive_creates_batch(env):
    ...
    PurchasingService.receive(
        po,
        [{
            "line_id": str(line.id), "quantity": "30",
            "batch_data": {"batch_identifier": "LOT-77"},
            "expiry_date": "2027-01-31",
        }],
        env["warehouse"], env["user"],
    )
...
def test_serialized_receive_requires_matching_serials(env):
    ...
    with pytest.raises(InventoryError, match="one serial per unit"):
        PurchasingService.receive(
            po, [{"line_id": str(line.id), "quantity": "2", "serials": ["SN-9"]}],
            env["warehouse"], env["user"],
        )
...
def test_api_full_cycle(env, api):
    ...
    resp = api.post('/api/v1/purchase-orders/', {
    ...
    # Receive partial
    resp = api.post(f'/api/v1/purchase-orders/{po_id}/receive/', {
