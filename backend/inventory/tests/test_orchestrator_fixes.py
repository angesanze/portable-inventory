...
def product_setup(company_setup):
    ...
    product = ProductModel.objects.create(
        company=company_setup, name="Orch Serial", sku=f"ORCH-{uuid.uuid4()}", profile="SERIALIZED"
    )
    return company_setup, loc, product
...
def test_orchestrator_field_mismatch_checkin(company_setup):
    ...
    product = ProductModel.objects.create(
        company=company, name="Orch Prod", sku=f"SKU-{uuid.uuid4()}", profile="SERIALIZED"
    )
    ...
    result = InventoryOrchestrator.handle_widget_movement(
        company=company, product_model=product, location=loc, data=data
    )
...
def test_orchestrator_fifo_fallback(company_setup):
    ...
    product = ProductModel.objects.create(
        company=company, name="Orch FIFO Prod", sku=f"SKU-FIFO-{uuid.uuid4()}", profile="SERIALIZED"
    )
