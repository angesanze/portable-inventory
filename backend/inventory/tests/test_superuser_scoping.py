...
class TestSuperUserScoping:
    def test_superuser_sees_all_records_by_default(self):
        ...
        company_a = Company.objects.create(name=f"Company A {unique_suffix}", license_code=f"LICA{unique_suffix[:4]}")
        product_a = ProductModel.objects.create(name="Product A", company=company_a, sku=f"SKU-A-{unique_suffix}")

        # Company B (Empty)
        company_b = Company.objects.create(name=f"Company B {unique_suffix}", license_code=f"LICB{unique_suffix[:4]}")
        
        # 2. Setup Superuser assigned to Company B
        superuser = User.objects.create_superuser(username=f"admin_b_{unique_suffix}", email="admin@b.com", password="password")
    ...
    def test_superuser_should_be_scoped_if_company_set(self):
        ...
        unique_suffix = uuid.uuid4().hex[:8]
         # 1. Setup Data
        company_a = Company.objects.create(name=f"Company A {unique_suffix}", license_code=f"LCA2{unique_suffix[:4]}")
        product_a = ProductModel.objects.create(name="Product A", company=company_a, sku=f"SKU-A-2-{unique_suffix}")

        company_b = Company.objects.create(name=f"Company B {unique_suffix}", license_code=f"LCB2{unique_suffix[:4]}")
        
        superuser = User.objects.create_superuser(username=f"admin_b_2_{unique_suffix}", email="admin@b.com", password="password")
        superuser.company = company_b
        superuser.save()
