...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for product in products:
            # Isolate per-product failures: one product raising (bad rule config,
            # transient DB error) must not abort monitoring for every other
            # product and tenant in the run.
            try:
                RuleEvaluator.evaluate_product(product)
                count += 1
            except Exception as exc:  # noqa: BLE001 — deliberately broad, logged and counted
                errors += 1
                self.stderr.write(f"Rule check failed for {product.sku} ({product.id}): {exc}")

        self.stdout.write(self.style.SUCCESS(f"Successfully checked rules for {count} products"))
        if errors:
            self.stdout.write(self.style.WARNING(f"{errors} product(s) failed rule evaluation"))
