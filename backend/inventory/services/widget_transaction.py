...
from ..models import Location, ProductModel, PhysicalProduct, WorkOrder
...
class WidgetTransactionService:
    ...
    def _build_dimension_payload(product, engine, data):
        ...
        # Values arrive nested under `dimension_values` (widget wire format);
        # fall back to the top level for legacy flat callers.
        dvals = data.get("dimension_values")
    ...
    def _build_batch_tracked_payload(product, operation, data, time_based_batch_data):
        ...
        if time_based_batch_data:
            if not batch_data:
                batch_ref = time_based_batch_data.get(
                    "batch_ref", f"TB-{uuid.uuid4().hex[:6].upper()}"
                )
                batch_data = {
    ...
    def process_transaction(company, api_key, pk, data):
        ...
        if operation == "produce_kit" and product.components.exists():
            ...
            with transaction.atomic():
                new_wo = WorkOrder.objects.create(
                    company=company, name=wo_name, product_model=product, status="OPEN"
                )
        ...
        # --- Dimension Engine: the quantity is derived from the formula, not the
        # raw `quantity` field (which the widget sends as 0). Compute it BEFORE
        # the positivity guard so a dimensional move isn't rejected as "Invalid
        # quantity" before the formula ever runs. ---
            ...
            if not physical_product and product.tracking_mode == "INDIVIDUAL":
                ...
                candidate = (
                    PhysicalProduct.objects.filter(
                        product_model=product, location=source, status="ACTIVE"
                    )
                    .exclude(reservations__status="ACTIVE")
                    .order_by(F("batch_date").asc(nulls_last=True), "identifier")
                    .first()
                )
