...
from ..models import (
    WorkOrder, ProductBatch, ProductModel, Location, ProductComponent,
    PhysicalProduct,
)
...
class WorkOrderService:
    ...
    def create_with_items(validated_data, items_data):
        ...
        warehouse = Location.objects.filter(
            company=company, type=LOCATION_TYPE_WAREHOUSE
        ).first()
            ...
            if physical_product_id:
                ...
                if not pp:
                    raise serializers.ValidationError(
                        {"items": "Physical product not found."}
                    )
                ...
                if not ProductModel.objects.filter(
                    id=product_model_id, company=company
                ).exists():
                    raise serializers.ValidationError(
                        {"items": "Product model not found."}
                    )
