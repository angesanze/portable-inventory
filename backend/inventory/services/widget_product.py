...
class WidgetProductService:
    ...
    def _build_product_payload(p, engine, stock_info, *, include_detail_extras=False):
        ...
        stock_display = engine.format_stock_display(
            StockService.get_tracker_status_counts(p)
            if not engine.returns_numeric_delta and p.engine_type == "tracker"
            else StockService.get_expiry_display_data(p, p.engine_config)
            if not engine.returns_numeric_delta and p.engine_type == "time_based"
            else total_val
        )
    ...
    def get_widget_products(company, location_id=None):
        ...
        """
        products = ProductModel.objects.filter(company=company).select_related("default_calculator")
        ...
        for p in products:
            try:
                ...
                # Show ALL products, including those at zero stock at this
                # location. The widget does check-IN, not just check-out, so a
                # brand-new product (or one emptied to zero) MUST remain
                # selectable to receive its first/next stock. Hiding zero-stock
                # items here made it impossible to load initial stock via the
                # widget — the user simply couldn't find the product.
        ...
        work_orders = WorkOrder.objects.filter(company=company, status="OPEN").select_related(
            "product_model"
        )
        ...
        if location_id:
            ...
            for wo in work_orders:
                has_items_here = (
                    wo.batches.filter(location_id=location_id, quantity__gt=0).exists()
                    or wo.physical_products.filter(
                        location_id=location_id, status="ACTIVE"
                    ).exists()
                )
                has_items_anywhere = (
                    wo.batches.filter(quantity__gt=0).exists()
                    or wo.physical_products.filter(status="ACTIVE").exists()
                )
            ...
            data.append(
                {
                    ...
                    "quantity": 0,
                    "strategy": None,
                    "calc_config": {"engine": "batch_manager", "ui_config": {}},
                    "components": [],
                }
            )
    ...
    def get_widget_product_details(company, pk, location_id=None):
        ...
        if work_order:
            ...
            batch_qs = ProductBatch.objects.filter(work_order=work_order).select_related(
                "product_model"
            )
            ...
            item_qs = PhysicalProduct.objects.filter(
            ...
            for item in item_qs:
                ...
                grouped_items[pm_id]["items"].append(
                    {"id": str(item.id), "identifier": item.identifier, "quantity": 1}
                )
        ...
        if product.engine_type == "bucket":
            ...
            batch_qs = ProductBatch.objects.filter(product_model=product, quantity__gt=0)
